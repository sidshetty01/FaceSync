...
export default function DemoSessionPage() {
  ...
  const createSession = async () => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000') + "/api/attendance/create_session", {
        method: "POST",
  ...
  const handleRecognize = useCallback(
    async (imageDataUrl: string) => {
      ...
      try {
        const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000') + "/api/attendance/real-mark", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
