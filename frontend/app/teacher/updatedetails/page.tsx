...
export default function TeacherUpdateStudentDetails() {
  ...
  const searchStudents = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000'}/api/teacher/students/search?${params}`, {
        headers: {
  ...
  const selectStudent = async (selectedStudent: Student) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000'}/api/teacher/student/${selectedStudent._id}`, {
        headers: {
          "Content-Type": "application/json",
          "X-User-Type": "teacher",
          "X-User-Email": localStorage.getItem("userEmail") || ""
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000'}/api/teacher/student/${student._id}`, {
        method: "PUT",
        headers: { 
          "Content-Type": "application/json",
          "X-User-Type": "teacher",
