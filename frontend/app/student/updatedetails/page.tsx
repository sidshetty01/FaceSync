...
export default function UpdateStudentDetails() {
  ...
  const fetchStudents = async (email: string, type: "student" | "teacher") => {
    try {
      let url = (process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000') + "/api/students";
      ...
      if (type === "student") {
        ...
        url = (process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000') + "/api/admin/students";
        headers["X-User-Email"] = email;
      }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000'}/api/students/${selectedStudent._id}`, {
        method: "PUT",
        headers,
        body: JSON.stringify({
  ...
  const handleDelete = async (studentId: string, studentName: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5000'}/api/students/${studentId}`, {
        method: "DELETE",
        headers: { 
          "Content-Type": "application/json",
